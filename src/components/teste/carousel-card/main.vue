<template>
  <div ref="rootRef" :class="carouselCardClazz" @mouseenter.stop="handleMouseEnter" @mouseleave.stop="handleMouseLeave">
    <div class="carousel-card-container">
      <transition v-if="arrowDisplay" name="carousel-card-arrow-left">
        <button v-show="(arrow === 'always' || hover) && (loop || activeIndex > 0)" type="button"
          class="carousel-card-arrow carousel-card-arrow-left" @mouseenter="handleButtonEnter('left')"
          @mouseleave="handleButtonLeave" @click.stop="throttledArrowClick(activeIndex - 1)">
          <i></i>
        </button>
      </transition>
      <transition v-if="arrowDisplay" name="carousel-card-arrow-right">
        <button v-show="(arrow === 'always' || hover) && (loop || activeIndex < items.length - 1)" type="button"
          class="carousel-card-arrow carousel-card-arrow-right" @mouseenter="handleButtonEnter('right')"
          @mouseleave="handleButtonLeave" @click.stop="throttledArrowClick(activeIndex + 1)">
          <i></i>
        </button>
      </transition>
...
import type { ResizableElement, ICarouselCardProps, CarouselCardItem, injectCarouselCardScope } from './carousel-card'
...
export default defineComponent({
  ...
  setup(props: ICarouselCardProps, { emit }) {
    ...
    const throttledArrowClick = throttle(300, (index: number) => {
      setActiveItem(index);
    }, { noTrailing: true });
